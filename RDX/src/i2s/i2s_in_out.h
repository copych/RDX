...
class  I2S_Audio {
  ...
  const i2s_bits_per_sample_t chn_bit_width = (i2s_bits_per_sample_t)CHANNEL_SAMPLE_BITS;
    ...
    inline int32_t              convertOutSample(float smp)       { 
      int32_t res = smp * float_to_int;
      if (res > 32767) {return 32767;}
      if (res < -32768) {return -32768;}
      return res+1; // disable auto-mute DAC by transmitting 1 instead of 0
    }
    ...
    inline IRAM_ATTR __attribute__((always_inline)) int32_t readSampleQ24_8(int n, int chan, BUF_TYPE* buf) { 
        return convertInSampleQ24_8(buf[_channel_num * n + chan]); 
    }

    inline IRAM_ATTR __attribute__((always_inline)) void writeSampleQ24_8(int32_t smp, int n, int chan, BUF_TYPE* buf) { 
        buf[_channel_num * n + chan] = convertOutSampleQ24_8(smp); 
    }
    ...
    */
    ...
    inline BUF_TYPE     convertOutSampleQ24_8(int32_t smp) { return (BUF_TYPE)(smp >> 8); } // Q24.8 -> 16-bit I2S
    ...
    inline int          convertInSampleQ24_8(BUF_TYPE smp) { return ((int32_t)smp) << 8; } // 16-bit I2S -> Q24.8


    void                writeBuffersQ24_8(int32_t* L, int32_t* R);
